...
char train_file[FILE_NAME_LENGTH], output_file[FILE_NAME_LENGTH];
char save_vocab_file[FILE_NAME_LENGTH], read_vocab_file[FILE_NAME_LENGTH];
char initfromFile[FILE_NAME_LENGTH];
char neighborFile[FILE_NAME_LENGTH];
char labelFile[FILE_NAME_LENGTH];
...
void loadNeighbors() 
{
    ...
    for (a = 0; a < vocab_size; a++){
      n2vvocab[a].index = a; n2vvocab[a].cn = 0; 
      for (b = 0; b < max_neighbors; b++) nbrs[a * max_neighbors + b] = -1; 
    }
    ...
    for (it=0; it<nnodes; it++) {
      ...
      if (debug_mode > 3) for (xb=0; xb<max_neighbors; xb++) printf("nbr%lld\n",nbrs[xb + index1*max_neighbors]);  
    }
...
void DestroyNet() {
  ...
  if (nbrs != NULL)     free(nbrs);
  if (syn0temp!=NULL)   free(syn0temp);
  if (temp!=NULL)       free(temp);
  if (templabel!=NULL)  free(templabel);
  if (syn1label!=NULL)  free(syn1label);  
}
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (cw) {
        ...
        // HIERARCHICAL SAMPLING CODE OMITTED
        // NEGATIVE SAMPLING
...
void TrainModel() {
  ...
  if (labelFile[0]!=0) loadLabelFile();
